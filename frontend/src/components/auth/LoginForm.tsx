...
import { loginWithEmail, getGoogleOAuthUrl, storeTokens } from "@/lib/auth/api";
  ...
  const { form, isSubmitting, showSpinner, serverError, handleSubmit, dismissError } =
    useAuthForm<LoginInput>({
      ...
      onSuccess: async (result) => {
        ...
        posthog.identify(authRes.user.id, {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="space-y-1">
        <h1 className="text-[28px] font-semibold tracking-tight text-[#0A0A0A]">Welcome back</h1>
        <p className="text-[14px] text-[#71717A]">Sign in to your Clairo account.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
        <div className="space-y-1.5">
          <label htmlFor="login-email" className="text-[13px] font-medium text-[#0A0A0A]">
            Email
          </label>
          <Input
            id="login-email"
          ...
          />
          {errors.email && (
            <p id="login-email-error" className="text-[12px] text-[#EF4444]" role="alert">
        ...
        <div className="flex justify-end">
          <Link
            href="#"
            tabIndex={-1}
            className="text-[12px] font-medium text-[#71717A] hover:text-[#0A0A0A] transition-colors"
          >
            Forgot password?
          </Link>
        </div>
