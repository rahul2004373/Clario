...
export function FeedbackWidget({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await fetch(`${apiUrl}/public/feedback`, {
        method: "POST",
        ...
        >
              ...
              {EMOJIS.map((emoji) => (
                <ToggleGroup.Item key={emoji.id} value={emoji.id} asChild>
                  <button
                    ...
                    disabled={success}
                    className={cn(
                      "relative rounded-full p-2 outline-none transition-colors focus-visible:ring-2 focus-visible:ring-zinc-900 dark:focus-visible:ring-zinc-300",
                      value === emoji.id
          ...
          <AnimatePresence mode="popLayout" initial={false}>
            {isExpanded && (
              ...
              >
                {success ? (
                  <div className="py-8 text-center text-sm font-medium text-green-600 dark:text-green-400">
